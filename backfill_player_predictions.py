    ...
    for i, date_str in enumerate(dates, start=1):
        ...
        try:
            ...
            if df is None or df.empty:
                # PROTECT EXISTING GOOD SNAPSHOTS. An "empty" result almost
                # always means the lineup API was unreachable, not that the
                # date had no games. If a populated snapshot already exists,
                # leave it alone. Only write an empty file if nothing's there.
                if snap.exists() and snap.stat().st_size > 50:
                    print(f"             ⚠ empty fetch — keeping existing snapshot intact")
                    n_skipped += 1
                else:
                    snap.write_text("")
                    n_empty += 1
