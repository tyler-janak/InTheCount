    ...
    for side in ("home", "away"):
        ...
        for _pid, pdata in players.items():
            ...
            if (pa not in (None, 0)) or (ab not in (None, 0)):
                ...
                h_n  = batting.get("hits")      or 0
                d_n  = batting.get("doubles")   or 0
                t_n  = batting.get("triples")   or 0
                hr_n = batting.get("homeRuns")  or 0
                tb_actual = int(h_n) + int(d_n) + 2 * int(t_n) + 3 * int(hr_n)
                hitters.append({
                    "mlb_id": mlb_id,
                    "player_name": name,
                    ...
                    "actual_doubles": batting.get("doubles"),
                    "actual_triples": batting.get("triples"),
                    "actual_total_bases": tb_actual,
...
PROJ_COLUMNS_KEEP = [
    ...
    "proj_runs", "proj_rbi", "proj_tb",
    "proj_ip", "proj_hits_allowed", "proj_runs_allowed",
    "lineup_status", "lineup_spot", "confidence", "confidence_score",
    "used_fallback",
    # Blend components — direct-count model vs (rate model × opportunity).
    # tune_blend.py grid-searches the optimal α per stat from these.
    "direct_K", "rate_K", "direct_BB", "rate_BB",
        ...
        if hitters_act.empty:
            ...
            hitters_graded = hitters_proj.merge(
                hitters_act[[
                    ...
                    "actual_doubles", "actual_triples", "actual_total_bases",
