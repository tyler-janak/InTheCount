...
def load_park_factors(filepath: str = "data/park_factors.csv") -> pd.DataFrame:
    ...
    # Historically the pipeline died silently for weeks because the first column
    # was being parsed as "﻿team" instead of "team".
    pf = pd.read_csv(path, encoding="utf-8-sig")
    pf.columns = [str(c).strip().lstrip("﻿") for c in pf.columns]

    if "team" not in pf.columns or "park_factor" not in pf.columns:
        raise ValueError(
            f"park_factors.csv must have columns: team, park_factor "
            f"(got: {list(pf.columns)})"
        )
...
def event_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["is_2b"] = (ev == "double").astype(int)
    df["is_3b"] = (ev == "triple").astype(int)
    # Total Bases per event:  1B=1, 2B=2, 3B=3, HR=4.
    # = is_hit (1 for any hit) + is_2b + 2*is_3b + 3*is_hr
    df["TB_event"] = df["is_hit"] + df["is_2b"] + 2 * df["is_3b"] + 3 * df["is_hr"]
