...
def fetch_schedule(target_date: str) -> list[dict]:
    """Fetch MLB schedule for target_date. Retries up to 4 times with
    exponential backoff so a single transient MLB API hiccup doesn't
    propagate up and silently skip step 8 of daily_update.py (leaving
    outputs/hitterspitchers_today.csv stale).
    """
    import time
    url = (
...
RATE_ENSEMBLE_BLEND_GLOBAL = 1.0     # multiplier applied to every per-stat α

PITCHER_BLEND_WEIGHTS = {
    "K":  0.50,   # rate model ≈ direct count -> equal blend
...
def score_pitchers(
    ...
    pitcher_rate_models: dict | None = None,
) -> pd.DataFrame:
    """
    ...
    for _, row in pitchers_today.iterrows():
        ...
        if RAW_MODEL_ONLY:
            ...
            # (already at book benchmark). Blending is usually better than
            # either side alone, especially on K and Hits-Allowed where
            # opportunity is solid but per-opportunity efficiency is the noisy
            # part. Set RATE_ENSEMBLE_BLEND_GLOBAL=0 to disable globally.
            if pitcher_rate_models and RATE_ENSEMBLE_BLEND_GLOBAL > 0:
                ip9 = ip / 9.0
                for proj_var, rate_key in (("K",  "K_per_9"),
                    ...
                    if a <= 0:
                        continue
                    bundle = pitcher_rate_models.get(rate_key)
                    if bundle is None:
                        continue
                    r = predict_model(bundle, feat)
                    if r is None:
                        continue
                    rate_count = max(0.0, float(r)) * ip9
                    if   proj_var == "K":  rate_K  = rate_count; proj_strikeouts   = (1 - a) * proj_strikeouts   + a * rate_count
        ...
        rows.append({
            ...
            "rate_K":    None if pd.isna(rate_K)  else round(float(rate_K),  3),
            "direct_BB": round(float(direct_BB), 3),
            "rate_BB":   None if pd.isna(rate_BB) else round(float(rate_BB), 3),
            "direct_H":  round(float(direct_H),  3),
...
def score_hitters(
    ...
    hitter_rate_models: dict | None = None,
) -> pd.DataFrame:
    """
    ...
    for _, row in hitters_today.iterrows():
        ...
        if RAW_MODEL_ONLY:
            ...
            # average with the direct one.
            if hitter_rate_models and RATE_ENSEMBLE_BLEND_GLOBAL > 0 and pa > 0:
                for proj_var, rate_key in (("H",  "H_per_PA"),
                                            ("HR", "HR_per_PA"),
                                            ("BB", "BB_per_PA"),
                                            ("K",  "K_per_PA"),
                                            ("TB", "TB_per_PA")):
                    ...
                    if a <= 0:
                        continue
                    bundle = hitter_rate_models.get(rate_key)
                    if bundle is None:
                        continue
                    r = predict_model(bundle, feat)
                    if r is None:
                        continue
                    rate_count = max(0.0, float(r)) * pa
                    if   proj_var == "H":  rate_h_hit  = rate_count; hits = (1 - a) * hits + a * rate_count
                    elif proj_var == "HR": rate_hr_hit = rate_count; hr   = (1 - a) * hr   + a * rate_count
        ...
        rows.append({
            ...
            "direct_BB": round(float(direct_bb_hit), 3),
            "rate_BB":   None if pd.isna(rate_bb_hit) else round(float(rate_bb_hit), 3),
            "direct_H":  round(float(direct_h_hit),  3),
            "rate_H":    None if pd.isna(rate_h_hit)  else round(float(rate_h_hit),  3),
            "direct_HR": round(float(direct_hr_hit), 3),
...
def run_projections(target_date: str | None = None) -> pd.DataFrame:
    ...
    # Per-opportunity rate models for the ensemble blend (skipped silently
    # when RATE_ENSEMBLE_BLEND_GLOBAL == 0 or the pickles don't exist yet).
    pitcher_rate_models: dict = {}
    hitter_rate_models:  dict = {}
    if RATE_ENSEMBLE_BLEND_GLOBAL > 0:
        pitcher_rate_models = load_models_count_only("pitcher", PITCHER_RATE_TARGETS)
        hitter_rate_models  = load_models_count_only("hitter",  HITTER_RATE_TARGETS)
        if pitcher_rate_models:
            print(f"  Pitcher rate models loaded: {list(pitcher_rate_models.keys())}  "
                  f"(global blend factor={RATE_ENSEMBLE_BLEND_GLOBAL:.2f})")
