...
BENCHMARK_MAE = {
    ...
    "rbi":            (0.55, 0.7),
    "pa":             (0.6, 0.8),
    "total_bases":    (0.9, 1.1),    # books target ~0.9 MAE on TB props
}
...
def section_player_accuracy(since: str | None) -> None:
    ...
    if not h.empty:
        ...
        for stat, proj_col, act_col, bench_key in [
            ...
            ("TB",     "proj_tb",         "actual_total_bases","total_bases"),
        ]:
            ...
            err = (pd.to_numeric(sub[proj_col]) - pd.to_numeric(sub[act_col])).abs()
            mae = float(err.mean())
            tol = 1.5 if stat == "TB" else (1.0 if stat in ("Hits", "Runs", "RBI", "Walks", "PA") else 0.5)
            within = float((err <= tol).mean())
            bench = BENCHMARK_MAE.get(bench_key)
